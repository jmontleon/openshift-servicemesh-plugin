...
export class EmptyGraphLayout extends React.Component<EmptyGraphLayoutProps, EmptyGraphLayoutState> {
  ...
  render(): React.ReactNode {
    if (this.props.isError) {
      return (
        <EmptyState id="empty-graph-error" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('Error loading graph')} icon={KialiIcon.Error} headingLevel="h5">
    ...
    if (this.props.isLoading) {
      return (
        <EmptyState id="empty-graph-is-loading" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('Loading graph')} headingLevel="h5" />
      );
    }
    ...
    if (this.props.namespaces?.length === 0) {
      return (
        <EmptyState id="empty-graph-no-namespace" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('No namespace is selected')} headingLevel="h5">
          <EmptyStateBody>
    ...
    if (isGraphEmpty && !this.props.isMiniGraph) {
      return (
        <EmptyState id="empty-graph" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('Empty Graph')} headingLevel="h5">
          <EmptyStateBody>
            {t(
    ...
    if (isGraphEmpty && this.props.isMiniGraph) {
      return (
        <EmptyState id="empty-mini-graph" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('Empty Graph')} headingLevel="h5">
