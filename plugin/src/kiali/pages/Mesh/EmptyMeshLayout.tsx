...
export class EmptyMeshLayout extends React.Component<EmptyMeshLayoutProps, EmptyMeshLayoutState> {
  ...
  render() {
    if (this.props.isError) {
      return (
        <EmptyState id="empty-mesh-error" variant={EmptyStateVariant.lg} className={emptyStateStyle} icon={KialiIcon.Error} titleText={t('Error loading Mesh')} headingLevel="h5">
    ...
    if (this.props.isLoading) {
      return (
        <EmptyState id="empty-mesh-is-loading" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('Loading Mesh')} headingLevel="h5">
        </EmptyState>
      );
    }
    ...
    if (isMeshEmpty && !this.props.isMiniMesh) {
      return (
        <EmptyState id="empty-mesh" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('Empty Mesh')} headingLevel="h5">
          <EmptyStateBody>
            {t(
    ...
    if (isMeshEmpty && this.props.isMiniMesh) {
      return (
        <EmptyState id="empty-mini-mesh" variant={EmptyStateVariant.lg} className={emptyStateStyle} titleText={t('Empty Mesh')} headingLevel="h5">
